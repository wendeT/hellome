...
def train_f():
	...
	while flag < len(data_y):
		...
		for i in range(len(data_y)):
			...
			model_y[i] = temp
			print '		Current target value at index is %d ' % (data_y[i]) 
			print '		Current model value at index is %d ' % (model_y[i]) 
			print ' 	Current flag value %d ' % (flag)
			if  model_y[i] == data_y[i]:
				flag += 1
				print '		Value matched flag incremented '
				print ' 	Current flag value %d ' % (flag)	
			else:
				print '	Failed '
